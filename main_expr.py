...
def parse_expression(IMAGE, net):
    # Overall pipeline
...
f = open(imgpath + 'listfile.txt')
r = open(imgpath + 'result.txt', 'w')
while True:
    line = f.readline()
    print line
    if not line:
        break
    filename, ans = line.split()
    expr, _ =  parse_expression(imgpath + filename, net)
    expr = expr.replace(' ', '')
    r.write('%s %s %s\n' % (filename, ans, expr))
    print ans, expr
